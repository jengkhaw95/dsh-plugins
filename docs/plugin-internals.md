...
Background material for these two plugins: how a bundle is shaped, how installing
and updating actually behaves, and the DSH gotchas that cost real time. Read this
before writing a plugin of your own.
...
Both follow the bundle convention described in the harness's own plugin
authoring guide, `docs/user/develop/basic/publish.md` in a DSH source checkout: a
...
`dsh plugin` forwards the rest of its arguments to pnpm inside the profile
directory, then adds each installed package to `dsh.profile.bundles`, so the
bundle's own `cordis.patch.yml` becomes the layer that mounts it. The plugin
subcommand never boots the harness — it only runs pnpm and rewrites the profile
manifest — so it is safe to run while DSH is up.
...
With the git install in place, `git reset --hard` updates this repo but the
profile keeps loading the pinned tarball, so a change looks like it did nothing.
Switch to the local link with:
...
Two failure modes cost real time while building these.
...
disable back into your profile's patch file** (`vendor/loader/src/index.ts`, the
disposal interceptor: `fiber.entry.options.disabled = true` followed by
...
[published pricing page](https://api-docs.deepseek.com/quick_start/pricing/):
peak is Monday–Friday 01:00–04:00 and 06:00–10:00 UTC, window ends exclusive,
everything else off-peak at half price.
